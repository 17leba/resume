...
import AnimationTimer from './animator'

function Game(canvasW, canvasH, context, canvasBgContext) {
    var that = this

    this.canvasWidth = canvasW
    this.canvasHeight = canvasH
    this.context = context
    this.canvasBgContext = canvasBgContext
    ...
    // cutDown sound
    this.cutSound = Config.mediaSource[0]
    // 开始音效
    this.startSound = Config.mediaSource[1]
    // 骰子音效
    this.diceSound = Config.mediaSource[2]
    // 跳跃音效
    this.jumperSound = Config.mediaSource[3]
    // 跑步音效
    this.runnerSound = Config.mediaSource[4]
    // resume sound
    this.resumeSound = Config.mediaSource[5]
    ...
    this.diceTwoPaintBehaivor = {
        ...
        execute: function(sprite, context, time) {
            ...
            if (sprite.diceAnimationTimer.isExpired()) {
                ...
                var timeOut = setTimeout(function executeAni() {
                    if (that.pauseGameTag) {
                        setTimeout(executeAni, 200)
                    } else {
                        self.roleStartMove()
                        that.diceLocReset()
...
Game.prototype = {
    ...
    drawDice: function(time) {
        ...
        this.diceOne.paint(this.context)
        this.diceOne.update(this.context, time)

        this.diceTwo.paint(this.context)
        this.diceTwo.update(this.context, time)
    },
    drawWalkMap: function() {
        ...
        // var mapBlockTop = new Sprite('map-block', new DrawStaticImage(Config.imgSource[0], Config.jsonObj['main']['map-block-top'])),
        var mapBlockBottom = new Sprite('map-block', new DrawStaticImage(Config.imgSource[0], Config.jsonObj['main']['map-block-bottom'])),
    ...
    drawEnclosure: function() {
        ...
        for (var j = 0; j < 8; j++) {
            enclosureHorizontal.left = j * 132 + 11
    ...
    drawJumper: function(time) {
        this.jumper.paint(this.context)
        if (this.roleIsMoving || this.roleIsAction) {
            this.jumper.update(this.context, time)
        }
    },
    drawRunner: function(time) {
        this.runner.paint(this.context)
        if (this.roleIsMoving) {
            this.runner.update(this.context, time)
        }
    },
    ...
    offScreenCanvasBg: function() {
        // walkMap（行走地图）
        this.canvasBgContext.drawImage(this.mapOffCanvas, this.walkMapLeft, this.walkMapTop)
        // enclosure（围栏）
        this.canvasBgContext.drawImage(this.enclosureVOffCanvas, 0, 0)
        this.canvasBgContext.drawImage(this.enclosureVOffCanvas, this.canvasWidth - 15, 0)
        this.canvasBgContext.drawImage(this.enclosureHOffCanvas, 0, 0)
        this.canvasBgContext.drawImage(this.enclosureHOffCanvas, 0, this.canvasHeight - 30)
        // trees（大树）
        this.canvasBgContext.drawImage(this.tree1OffCanvas, 0, 0)
        this.canvasBgContext.drawImage(this.tree2OffCanvas, this.canvasWidth - 95 * 2, this.canvasHeight - 120)
        // tent（帐篷）
        this.canvasBgContext.drawImage(this.tentOffCanvas, this.canvasWidth - 160, 0)
        // firewood（柴火）
        this.canvasBgContext.drawImage(this.firewoodOffCanvas, this.canvasWidth - 128, 160)
        // cask（木桶）
        this.canvasBgContext.drawImage(this.caskOffCanvas, 60, 120)
        // chair（椅子）
        this.canvasBgContext.drawImage(this.chairOffCanvas, 50, 400)
    },
